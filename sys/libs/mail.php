<?
define("EmailNewLine", "\r\n"); 
define("EmailXMailer", "framework::apex (Nir Gavish) via PHP-EMAIL,v1.1 (William Fowler)"); 
...
//** FALSE is returned. 
  ...
  function Attach($pathtofile, $original_name=null, $mimetype=null) 
  { 
...
//** exist the attachment is not created and FALSE is returned. 
    ...
    $attachment = new EmailAttachment($pathtofile, $original_name, $mimetype); 
...
//** type is given the default 'application/octet-stream' is used. 
  ...
  function EmailAttachment($pathtofile=null, $filename=null, $mimetype=null) 
  { 
...
//** if an attachment then add the appropriate disposition and file name(s).
    ...
    {
        $name = $this->FileName;
        if($name==null){$name=basename($this->FilePath);}
      $header .= " name=\"" . $name . "\"" . EmailNewLine .
